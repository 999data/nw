...
//	unint32_t port_num=21; // port number for Rx
	...
	Address RxAddress(InetSocketAddress (csmaInterfaces.GetAddress(1), 21)); //Rx obtaining a IPv4 address, and the port number to listen
...
// set up address and port number to  send TCP traffic to the server
		Address TxAddress(InetSocketAddress(csmaInterfaces.GetAddress(1),21));
...
csma.EnableAsciiAll(ascii.CreateFileStream ("ftp_trace.tr"));
